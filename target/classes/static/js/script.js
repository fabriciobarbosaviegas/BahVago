document.addEventListener("DOMContentLoaded", () => {
    ...
    botao.addEventListener("click", async () => {
        const card = botao.closest(".admin-review-item-box");
        ...
        try {
            const response = await fetch(`/avaliacoes/${id}/responder`, {
                method: "POST",
                headers: {
            ...
            card.querySelector(".admin-reply-form-zone").style.display = "none";
 
            const respostaDiv = document.createElement("div");
            const hoje = new Date().toLocaleDateString("pt-BR");    
            respostaDiv.className = "hotel-response";
            respostaDiv.innerHTML = `
                <div class="hotel-response-header">
                    <strong>Resposta do Hotel</strong>
                    <small>${hoje}</small>
                </div>
                <p>${resposta}</p>`;

                const comentario = card.querySelector(".review-body-text-content");
                comentario.insertAdjacentElement("afterend", respostaDiv);
        } catch (e) {
    ...
    const reviewCards = document.querySelectorAll(".admin-review-item-box");
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            reviewCards.forEach(card => {
                ...
                const respondida = card.dataset.respondida === "true";
                let mostrar = true;
                switch (filter) {
                    case "pending":
                        mostrar = !respondida;
            ...
            card.classList.toggle("hidden-review", !mostrar);
            });
        });
    });
